...
class ArticleCategorizationController extends Controller
{
    ...
    public function listNews(Request $request,$path,$page=0)
    {
        ...
        //当前栏目信息缓存
        $thistypeinfo=Cache::remember('thistypeinfos_'.$typeid,  config('app.cachetime')+rand(60, config('app.cachetime')+rand(60,60*24)), function() use($typeid){
        ...
        if($thistypeinfo->mid==0)
        {
            ...
            return view('mobile.list_article',compact('thistypeinfo','pagelists','paihangbangs','investment_types','latestbrands','hotbrandnews','toparticlenavs'));
        }else{
            //品牌列表页
            //获取品牌顶级栏目分类并缓存
            $topbrandtypeinfos=Cache::remember('topbrandtypeinfos', config('app.cachetime')+rand(60,60*24), function (){
                return Arctype::where('mid',1)->where('reid',0)->take(25)->where('id','<>',596)->orderBy('sortrank','desc')->get(['id','typename','real_path']);
            });
            //获取当前栏目友情链接并缓存
            ...
            return view('mobile.brands',compact('thistypeinfo','topbrandtypeinfos','sontypes','pagelists','paihangbangs','investment_types','latestbrands','latestbrandnews','flinks','provinces','province_ids','thistypeinforeid','investment_ids','type_investment_types','newbrands'));
        }
    ...
    public function projectBrandLists($path,$tid='',$zid='',$page=0)
    {
        ...
        return view('mobile.project_brands',compact('thistypeinfo','topbrandtypeinfos','sontypes','pagelists','paihangbangs','investment_types','latestbrands','latestbrandnews','flinks','provinces','province_ids','thistypeinforeid','investment_ids','type_investment_types','newbrands','touzipath','project_title'));
     ...
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View
     */
    public function CityBrandLists($path,$path2='',$page=0,$tid='',$zid='')
    {
        is_numeric($page)?:abort(404);
        $typeid=Arctype::where('real_path',$path)->value('id')?:abort(404);
        $thistypeinfo=Cache::remember('thistypeinfos_'.$typeid,  config('app.cachetime')+rand(60,60*24), function() use($typeid){
        ...
        if ($thistypeinfo->reid)
        {
            ...
            //当前栏目品牌分页处理
            $pagelists=Brandarticle::where('province_id',Area::where('name_path',$path2)->value('id'))->where('typeid',$typeid)->orderBy('id','desc')->distinct()->paginate($perPage = 10, $columns = ['*'], $pageName = 'p', $page);
            $paihangbangs= Cache::remember('phb'.$thistypeinfo->id,  config('app.cachetime')+rand(60,60*24), function() use($thistypeinfo){
                return Brandarticle::where('typeid',$thistypeinfo->id)->take('10')->orderBy('click','desc')->get(['id','brandname','litpic','brandnum','tzid']);
            });
            //获取当前栏目最新品牌并缓存
            ...
            //当前栏目品牌分页处理
            $pagelists=Brandarticle::where('province_id',Area::where('name_path',$path2)->value('id'))->whereIn('typeid',Arctype::where('mid',1)->where('reid',$thistypeinfo->id)->pluck('id'))->orderBy('click','desc')->distinct()->paginate($perPage = 10, $columns = ['*'], $pageName = 'p', $page);
            //品牌分类排行榜
            $paihangbangs= Cache::remember('phb'.$thistypeinfo->id,  config('app.cachetime')+rand(60,60*24), function() use($thistypeinfo){
                return Brandarticle::whereIn('typeid',Arctype::where('mid',1)->where('reid',$thistypeinfo->id)->pluck('id'))->take('10')->orderBy('click','desc')->get(['id','brandname','litpic','brandnum','tzid']);
            });
     ...
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View
     */
    public function CityInverBrandLists($path,$path2='',$tid='',$zid='',$page=0)
    {
        is_numeric($page)?:abort(404);
        $typeid=Arctype::where('real_path',$path)->value('id')?:abort(404);
        $thistypeinfo=Cache::remember('thistypeinfos_'.$typeid,  config('app.cachetime')+rand(60,60*24), function() use($typeid){
    ...
    public function articleIndex()
    {
        ...
        return view('mobile.article_index',compact('hotnews','chuangyeasks','chuangyemiddleasks','chuangyemidbottomasks','newsphs',
